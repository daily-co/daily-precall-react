...
export const MicCheck: React.FC = () => {
  ...
  const { microphones, micState, hasMicError, setMicrophone, currentMic } =
    useDevices();
  ...
  const updateMicrophone = (ev: React.ChangeEvent<HTMLSelectElement>) => {
    setMicrophone(ev.target.value).catch((err) => logger.error(err));
  };
      ...
      {microphones.length > 0 && (
        <form>
          ...
          <select
            ...
            defaultValue={currentMic?.device.label}
          >
            {microphones.map((mic) => (
              <option
